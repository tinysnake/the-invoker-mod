...
//		this.setLightValue(0.5F);
	...
	{
		blockIcons = new IIcon[ItemTotem.NAMES.length - 1];
...
//	}
	...
	@Override
	protected void dropItem(World world, int x, int y, int z, Block neighborBlock, int metadata)
	{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (EvilTouchMisc.isPlayHoldingEvilTouch(player))
		{
			...
			if (tt.getIsGhostBlock() && world.getBlock(x, y - 1, z) == this)
			{
			...
			ItemStack dropItemStack = new ItemStack(TIItems.totem, 1, tt.getBlockMetadata());
			this.dropBlockAsItem(world, x, y, z, dropItemStack);
